...
api = "https://5d76bf96515d1a0014085cf9.mockapi.io/product" ;
  ...
  for (let i = 0; i < res.length; i++) {
    ...
    if (res[i].isAccessory === false) {
      mens_womens_clothing.innerHTML += `
          <div id=card${[i]} onclick="productDetailsPagePageRouter(card${[i]})"  class="product-card">
...
function productDetailsPagePageRouter(res) {
  ...
  // product details page
  function productDetailsPage(response) {
  ...
  `;
    ...
    // other details
    detailsImgSection(response);
  }

  // product details page details section
function detailsImgSection(response) {
    let imgSection = document.querySelector(".img-div");
  
    let detailSection = document.getElementById("details-section");
    imgSection.innerHTML = `  <img id='detailsBigImg' src=${response.preview} alt=${response.name}>`;
  
    detailSection.innerHTML = `
  <h1 id="product-titile">${response.name}</h1>
  <h1 id="product-brand">${response.brand}</h1>
    <h4 class="product-price-section">
        Price: Rs 
        <p id="product-price">${response.price}.00 </p>
    </h4>
  ...
  //product details page small img border setting
